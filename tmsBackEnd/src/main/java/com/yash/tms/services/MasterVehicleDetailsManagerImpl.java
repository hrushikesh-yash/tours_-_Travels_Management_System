...
public class MasterVehicleDetailsManagerImpl implements MasterVehicleDetailsManager{
	...
	public MasterVehicleDetails findById(int vehicleId) {
		...
		try {
				MasterVehicleDetails vehicleDetails=masterVehicleDetailsDao.findById(vehicleId).get();
				//orElseThrow(() -> new RecordNotfoundException("Vehicle Details Id not found."));
