...
public class MappingVehicleDriverManagerImpl implements MappingVehicleDriverManager {
	...
	public MappingVehicleDriverDetails findById(int mappingVehicleDriverDetailsId) {
		...
		try {
			MappingVehicleDriverDetails  mappingVehicleDriverDetails=mappingVechileDriverRepository.findById(mappingVehicleDriverDetailsId).get();
			//orElseThrow(() -> new RecordNotfoundException("Mapping of vechile driver Id not found."));
